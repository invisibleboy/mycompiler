...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddHarwell.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  int  pr /* verbosity level */)
{
    ...
    if (pr>0) {
	(void) fprintf(stdout,"%s: type %s, %d rows, %d columns, %d entries\n", key,
	mxtype, nrow, ncol, nnzero);
	if (pr>1) (void) fprintf(stdout,"%s\n", title);
    }

    /* Check matrix type */
    if (mxtype[0] != 'R' || mxtype[1] != 'U' || mxtype[2] != 'A') {
	(void) fprintf(stderr,"%s: Illegal matrix type: %s\n", key, mxtype);
	return(0);
    }
	...
	if (pr>0) (void) fprintf(stdout,"%d right-hand side(s)\n", nrhs);
    } else {
	nrhs = 0;
    }
	...
	*n = (1 << (lny - 1)) + nrhs;
    ...
    if (colptr[0] != 0) {
	(void) fprintf(stderr,"%s: Unexpected colptr[0] (%d)\n",key,colptr[0]);
	FREE(colptr);
	FREE(rowind);
	return(0);
    }
