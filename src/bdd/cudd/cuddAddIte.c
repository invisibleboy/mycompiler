...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddAddIte.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  DdNode * h)
{
    ...
    /* Check cache. */
    r = cuddConstantLookup(dd,Cudd_addIteConstant,f,g,h);
    if (r != NULL) {
        return(r);
    }
	...
	cuddCacheInsert(dd, Cudd_addIteConstant, f, g, h, DD_NON_CONSTANT);
	return(DD_NON_CONSTANT);
    }
    ...
    cuddCacheInsert(dd,Cudd_addIteConstant,f,g,h,t);
    return(t);
  ...
  DdNode * g)
{
	...
	Cudd_addEvalConst;
    r = cuddConstantLookup(dd,cacheOp,f,g,g);
    if (r != NULL) {
        return(r);
    }
  ...
  DdNode * g)
{
	...
	if (Cudd_IsConstant(g)) return(cuddV(f) <= cuddV(g));
  ...
  DdNode * h)
{
    ...
    /* Check cache. */
    r = cuddCacheLookup(dd,Cudd_addIte,f,g,h);
    if (r != NULL) {
        return(r);
    }
    ...
    cuddCacheInsert(dd,Cudd_addIte,f,g,h,r);
