...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddAPI.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  int  multiplicity /* how many ZDD variables are created for each BDD variable */)
{
    ...
    */
	...
	for (i = 0; i < dd->size; i++) {
	    MtrNode *node;
	    node = Cudd_MakeZddTreeNode(dd, i * multiplicity, multiplicity,
					MTR_DEFAULT);
...
/**Function********************************************************************
  ...
  Synopsis    [Reads the maxCache parameter of the manager.]

  Description []

  SideEffects [None]

  SeeAlso     [Cudd_SetMaxCache Cudd_ReadMaxCache]
...
/**Function********************************************************************
  ...
  Synopsis    [Sets the maxCache parameter of the manager.]

  Description [Sets the maxCache parameter of the manager. This parameter
  is normally automatically determined by the package based on the total
  number of nodes and the parameter maxCacheHard. Applications should
  normally change maxCacheHard rather than maxCache if they want to
  explicitly control the trade-off of memory versus speed.]

  SideEffects [None]

  SeeAlso     [Cudd_ReadMaxCache Cudd_SetMaxCacheHard]
  ...
  FILE * fp)
{
    ...
    if (retval == EOF) return(0);
    retval = fprintf(fp,"Used buckets in unique table: %.2f%%\n",
...
/**Function********************************************************************
  ...
  Description [Reports the number of nodes in BDDs and ADDs. This
...
/**Function********************************************************************
  ...
  Prints on stdout reordering method and initial size.
  Returns 1 if successful; 0 otherwise.]
  ...
  void *data)
{
    ...
    retval = fprintf(stdout,"Reordering with ");
    if (retval == EOF) return(0);
    switch (method) {
	...
	retval = fprintf(stdout,"converging ");
	if (retval == EOF) return(0);
	break;
    default:
	break;
	...
	retval = fprintf(stdout,"random");
	break;
    case CUDD_REORDER_SIFT:
    case CUDD_REORDER_SIFT_CONVERGE:
	retval = fprintf(stdout,"sifting");
	break;
    case CUDD_REORDER_SYMM_SIFT:
    case CUDD_REORDER_SYMM_SIFT_CONV:
	retval = fprintf(stdout,"symmetric sifting");
	break;
    case CUDD_REORDER_GROUP_SIFT:
    case CUDD_REORDER_GROUP_SIFT_CONV:
	retval = fprintf(stdout,"group sifting");
	break;
    ...
    case CUDD_REORDER_WINDOW3_CONV:
    case CUDD_REORDER_WINDOW4_CONV:
	retval = fprintf(stdout,"window");
	break;
    case CUDD_REORDER_ANNEALING:
	retval = fprintf(stdout,"annealing");
	break;
    case CUDD_REORDER_GENETIC:
	retval = fprintf(stdout,"genetic");
	break;
    case CUDD_REORDER_LINEAR:
    case CUDD_REORDER_LINEAR_CONVERGE:
	retval = fprintf(stdout,"linear sifting");
	break;
    case CUDD_REORDER_EXACT:
	retval = fprintf(stdout,"exact");
	break;
    default:
	return(0);
    }
    ...
    retval = fprintf(stdout,": from %ld to ... ", Cudd_ReadNodeCount(dd));
    if (retval == EOF) return(0);
    fflush(stdout);
    return(1);
...
/**Function********************************************************************
  ...
  Prints on stdout final size and reordering time.
  Returns 1 if successful; 0 otherwise.]
  ...
  void *data)
{
    ...
    retval = fprintf(stdout,"%ld nodes in %g sec\n",
		     Cudd_ReadNodeCount(dd), totalTimeSec);
    if (retval == EOF) return(0);
    else return(1);
