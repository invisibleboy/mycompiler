...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddLinear.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  DdManager * table)
{
	...
	for (j = 0; j < wordsPerRow; j++) {
	    ...
	    for (k = 0; k < BPL; k++) {
		retval = fprintf(stdout,"%ld",word & 1);
  ...
  Move * prevMoves)
{
    ...
    */
	...
	if (L != checkL) {
	    (void) fprintf(stdout, "checkL(%d) != L(%d)\n",checkL,L);
	}
#endif
...
#ifdef DD_DEBUG
	    if (newsize != size) {
		(void) fprintf(stdout,"Change in size after identity transformation! From %d to %d\n",size,newsize);
	    }
#endif
  ...
  Move * prevMoves)
{
	...
	if (R != checkR) {
	    (void) fprintf(stdout, "checkR(%d) != R(%d)\n",checkR,R);
	}
#endif
...
#ifdef DD_DEBUG
	    (void) fprintf(stdout,"Unforseen event in ddUndoMoves!\n");
#endif
...
#pragma pointer_size restore
    ...
    DdNode *g,*next,*last = NULL,*previous;
    DdNode *tmp;
#if DD_DEBUG
    int    count, idcheck;
#endif
	...
	*/
	...
	for (i = 0; i < yslots; i++) {
		...
		f = next;
	    } /* while there are elements in the collision list */
	} /* for every collision list */

#if DD_DEBUG
#if 0
	(void) fprintf(stdout,"Linearly combining %d and %d\n",x,y);
#endif
	count = 0;
	idcheck = 0;
	for (i = 0; i < yslots; i++) {
...
cuddLinearOutOfMem:
    (void) fprintf(stderr,"Error: cuddLinearInPlace() out of memory\n");
