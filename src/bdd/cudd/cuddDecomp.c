...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddDecomp.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
...
static double CountMinterms ARGS((DdNode * node, double max, st_table * mintermTable));
  ...
  st_table * distanceTable)
{
    ...
    */
	...
	ST_OUT_OF_MEM) {
	(void) fprintf(stdout, "st table insert failed\n");
	return(0);
  ...
  st_table * mintermTable)
{
    ...
    /* Recur on the children. */
    minNv = CountMinterms(Nv, max, mintermTable)/2.0;
    if (minNv == -1.0) return(-1.0);
    minNnv = CountMinterms(Nnv, max, mintermTable)/2.0;
  ...
  st_table * cacheTable)
{
	...
	fprintf(stdout, "Cache insert failed\n");
	FREE(factors);
	return NULL;
    }
  ...
  int switched)
{
	...
	if (st_insert(ghTable, (char *)Cudd_Regular(x), (char *)(long)value) == ST_OUT_OF_MEM) {
	    fprintf(stdout, "gTable insert failed\n");
	    dd->errorCode = CUDD_MEMORY_OUT;
	    return NULL;
	}
	return(factors);
  ...
  st_table * mintermTable)
{
    ...
    N = Cudd_Regular(node);
    if (!st_lookup(distanceTable, (char *)N, (char **)&nodeStat)) {
	(void) fprintf(stdout, "Not in table, Something wrong\n");
	return(NULL);
    }
  ...
  DdNode ** c2)
{
	...
	*c2 = DD_ONE(dd);
    ...
    minterms = CountMinterms(f, max, mintermTable);
