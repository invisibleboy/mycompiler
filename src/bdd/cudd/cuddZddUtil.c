...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddZddUtil.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  int  pr)
{
    ...
    if (f == empty && pr > 0) {
	(void) fprintf(stdout,": is the empty ZDD\n");
	(void) fflush(stdout);
	return(1);
    }
	...
	(void) fprintf(stdout,": %d nodes %g minterms\n", nodes, minterms);
	if (pr > 2)
...
#if SIZEOF_VOID_P == 8
    (void) fprintf(stdout, "ID = 0x%lx\tindex = %d\tr = %d\t",
	(unsigned long)f / (unsigned long) sizeof(DdNode), f->index, f->ref);
#else
    (void) fprintf(stdout, "ID = 0x%x\tindex = %d\tr = %d\t",
	(unsigned)f / (unsigned) sizeof(DdNode), f->index, f->ref);
#endif

    n = cuddT(f);
    if (Cudd_IsConstant(n)) {
        (void) fprintf(stdout, "T = %d\t\t", (n == base));
	T = 1;
    } else {
#if SIZEOF_VOID_P == 8
        (void) fprintf(stdout, "T = 0x%lx\t", (unsigned long) n /
...
#if SIZEOF_VOID_P == 8
        (void) fprintf(stdout, "E = 0x%lx\n", (unsigned long) n /
		      (unsigned long) sizeof(DdNode));
#else
        (void) fprintf(stdout, "E = 0x%x\n", (unsigned) n / (unsigned) sizeof(DdNode));
#endif
	E = 0;
    }
