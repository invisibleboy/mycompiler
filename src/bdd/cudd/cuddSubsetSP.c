...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddSubsetSP.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
...
static void CreateTopDist ARGS((st_table *pathTable, int parentPage, int parentQueueIndex, int topLen, DdNode **childPage, int childQueueIndex, int numParents));
static void CreateBotDist ARGS((DdNode *node, st_table *pathTable, unsigned int *pathLengthArray));
static st_table * CreatePathTable ARGS((DdNode *node, unsigned int *pathLengthArray));
static unsigned int AssessPathLength ARGS((unsigned int *pathLengthArray, int threshold, int numVars, unsigned int *excess));
  ...
  int  hardlimit /* flag determining whether thershold should be respected strictly */)
{
    ...
    if (f == NULL) {
	fprintf(stderr, "Cannot partition, nil object\n");
	return(NULL);
    }
    ...
    pathTable = CreatePathTable(f, pathLengthArray);
    ...
    *excess = 0;
...
#ifdef DD_DEBUG
	fprintf(stdout, "Path length array\n");
	...
	for (i = 0; i < (numVars+1); i++) {
	    if (pathLengthArray[i])
		fprintf(stdout, "%d ",pathLengthArray[i]);
	}
	fprintf(stdout, "\n");
	fprintf(stdout, "Maxpath  = %d, Thresholdreached = %d\n", maxpath,
	       info->thresholdReached);
#endif

	N = Cudd_Regular(f);
	if (!st_lookup(pathTable, (char *)N, (char **)&nodeStat)) {
	    fprintf(stderr, "Something wrong, root node must be in table \n");
	    exit(-1);
	} else {
		...
		fprintf(stderr, "All computations are bogus, since root has path length greater than max path length within threshold %d, %d\n", maxpath, pathLength);
...
#ifdef DD_DEBUG
	fprintf(stdout, "Hits = %d, New==Node = %d, NumCalls = %d\n", hits,
	       thishit, numCalls);
#endif
  ...
  int  numParents /* number of parents to process in this recursive call */)
{
    ...
    if ((queuePages[parentPage] == childPage) && (parentQueueIndex ==
						  childQueueIndex)) {
	fprintf(stderr, "Shouldnt happen that they are equal\n");
    }
	...
	CreateTopDist(pathTable, parentPage, parentQueueIndex, topLen,
		      childPage, childQueueIndex, childrenCount);
    }
  ...
  unsigned int * pathLengthArray /* array that stores number of nodes belonging to a particular path length. */)
{
    ...
    if (!st_lookup(pathTable, (char *)N, (char **)&nodeStat)) {
	fprintf(stderr, "Something wrong, the entry doesn't exist\n");
	exit(-1);
    }
	...
	if (Cudd_IsConstant(realChild)) {
	    ...
	    if (!st_lookup(pathTable, (char *) regChild,
			   (char **)&nodeStatChild)) {
		fprintf(stderr, "Something wrong, node in table should have been created in top dist proc.\n");
		exit(-1);
	    }

	    if (nodeStatChild->oddBotDist == MAXSHORTINT) {
		if (nodeStatChild->evenBotDist == MAXSHORTINT) {
		    CreateBotDist(realChild, pathTable, pathLengthArray);
		} else {
		    fprintf(stderr, "Something wrong, both bot nodeStats should be there\n");
		    exit(-1);
		}
	    }
		 ...
		 *  of child to get odd parity distance and add 1 to odd distance
		 *  of child to get even parity distance Change distance of
		 *  current node only if the calculated distance is lesser than
		 *  existing distance,
		 */
		if (nodeStatChild->oddBotDist != MAXSHORTINT)
		...
		/* if parity on the edge then add 1 to even distance
		 * of child to get even parity distance and add 1 to odd distance
		 * of child to get odd parity distance Change distance of current
		 * node only if the calculated distance is lesser than existing
		 * distance,
		 */
		if (nodeStatChild->evenBotDist != MAXSHORTINT)
		    botDist = nodeStatChild->evenBotDist + 1;
  ...
  unsigned int * pathLengthArray /* array of path lengths to store nodes labeled with the various path lengths */)
{
    ...
    *(currentQueuePage + queuePageIndex) = node;
	...
	fprintf(stderr, "Something wrong, the entry exists but didnt show up in st_lookup\n");
	exit(-1);
    }
    ...
    CreateTopDist(pathTable, parentPage, parentQueueIndex, (int) topLen,
		  childPage, childQueueIndex, numParents);
    if (memOut) {
	fprintf(stderr, "Out of Memory and cant count path lengths\n");
	goto OUT_OF_MEM;
    }
     ...
     * constant
     */
    CreateBotDist(node, pathTable, pathLengthArray);
...
OUT_OF_MEM:
    (void)fprintf(stderr, "Out of Memory, cant allocate pages\n");
    memOut = 1;
    return NULL;
...
/**Function********************************************************************
  ...
  Synopsis    [Chooses the maximum allowable path length os nodes under the
  threshold]
  ...
  unsigned int * excess /* number of nodes labeled maxpath required in the subset */)
{
	...
	*excess = temp + pathLengthArray[maxpath];
    ...
    if (!maxpath) {
	fprintf(stderr, "Path Length array seems to be all zeroes, check\n");
    }
    return(maxpath);
  ...
  st_table * subsetNodeTable /* table storing computed results */)
{
    ...
    char *entry;
    unsigned int topid;
    unsigned int childPathLength, oddLen, evenLen, NnvPathLength = 0,
      NvPathLength = 0;
	...
	if (childPathLength > info->maxpath) {
		...
		/* if the node labeled with maxpath is found in the maxpathTable,
		 * use it to build the subset BDD.
		 */
		if (st_lookup(info->maxpathTable, (char *)regChild,
			      (char **)&entry)) {
		    /* when a node that is already been chosen is hit, the quest
		     * for a complete path is over
		     */
		    if (info->findShortestPath) {
			info->findShortestPath = 0;
		    }
		    ...
		    /* if node is not found in the maxpathTable and the threshold
		     * has been reached, then if the path needs to be completed,
		     * continue. Else replace the node with a zero.
		     */
		    ...
		    } else { /* threshold hasn't been reached , need
				the node */
			if (st_insert(info->maxpathTable, (char *)regChild,
				      (char *)NIL(char)) == ST_OUT_OF_MEM) {
			    memOut = 1;
