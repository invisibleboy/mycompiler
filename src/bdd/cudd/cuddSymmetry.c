...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddSymmetry.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  int  upper)
{
	...
	if (table->subtables[i].next != (unsigned) i) {
	    x = i;
	    (void) fprintf(stdout,"Group:");
	    do {
		(void) fprintf(stdout,"  %d",table->invperm[x]);
		TotalSymm++;
		gbot = x;
    ...
    (void) fprintf(stdout,"Total Symmetric = %d\n",TotalSymm);
    (void) fprintf(stdout,"Total Groups = %d\n",TotalSymmGroups);
...
#pragma pointer_size restore
    ...
    */
	...
	(void) fprintf(stdout,"Found symmetry! x =%d\ty = %d\tPos(%d,%d)\n",
	    xindex,yindex,x,y);
    }
#endif
  ...
  int  upper)
{
    ...
    (void) fprintf(stdout,"#:G_SIFTING %8d: symmetric groups",symgroups);
#endif
