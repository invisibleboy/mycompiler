...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddSubsetHB.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
...
/**Function********************************************************************
  ...
  children. Starting at root, annotate each node with the number of minterms (in
  terms of the total number of variables specified - numVars), number of nodes
  taken by the DAG rooted at this node and number of additional nodes taken by
  the child that has the lesser minterms. The child with the lower number of
  minterms is thrown away and a dyanmic count of the nodes of the subset is
  kept. Once the threshold is reached the subset is returned to the calling
  procedure.] 
  ...
  int  threshold /* threshold size for the subset */)
{
    ...
    if (f == NULL) {
	fprintf(stdout, "Cannot subset, nil object\n");
	return(NULL);
    }
	...
	(void) fprintf(stdout, "Out-of-memory; Cannot subset\n");
	return(0);
    }
	...
	fprintf(stdout, "Something is wrong, ought to be node quality table\n");
    }
    ...
    *size = numNodes;
    ...
    /* insert the constant */
    cuddRef(one);
    if (st_insert(storeTable, (char *)Cudd_ReadOne(dd), NIL(char))== ST_OUT_OF_MEM ){
      fprintf(stdout, "Something wrong, st_table insert failed\n");
    }
    /* table to store approximations of nodes */
    approxTable = st_init_table(st_ptrcmp, st_ptrhash);
    subset = (DdNode *)BuildSubsetBdd(dd, f, size,
...
OUT_OF_MEM:
    (void) fprintf(stderr, "Out-of-memory; couldn't count BDD minterms.\n");
    memOut = 1;
    return(NULL);
  ...
  double  max /* maximum number of variables */)
{
	...
	fprintf(stdout, "Something wrong, this node should exist in node quality table\n");
	return(0);
    }
	...
	if (st_lookup(table, (char *)Nv, (char **)&dummyNv) == 1)
	    ...
	    fprintf(stdout, "Something wrong, this node should exist in node quality table and is not constant\n");
	    return(0);
	}
    }
...
OUT_OF_MEM:
    (void) fprintf(stderr, "Out-of-memory; couldn't count BDD nodes.\n");
    memOut = 1;
    return(0);
  ...
  st_table * approxTable)
{
	...
	fprintf(stdout, "Something is wrong, ought to be in node quality table\n");
    }
    ...
    if (!Cudd_IsConstant(Nv)) {
        ...
        if (!st_lookup(visitedTable, (char *)Nv,
		       (char **)&currNodeQualT)) {
		fprintf(stdout,"Something wrong, couldnt find nodes in node qualitytable\n");
		return(NULL);
	    }
	else {
	...
	if (!st_lookup(visitedTable, (char *)Nnv, (char **)&currNodeQualE)) {
	    fprintf(stdout,"Something wrong, couldnt find nodes in node quality table\n");
	    return(NULL);
	} else {
    ...
    *size = (*(size)) - (int)*(currNodeQual->lightChildNodesPointer);
	...
	if (N !=  Cudd_Regular(neW)) {
  	    if (st_lookup(approxTable, (char *)node, (char **)&dummy)) {
	        fprintf(stdout, "This node shouldnt be in the approximated table\n");
	    } else {
	        cuddRef(neW);
