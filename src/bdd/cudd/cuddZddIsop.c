...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddZddIsop.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  DdNode ** zdd_I)
{
	...
	*zdd_I = zdd_one;
    ...
    if (U == zero) {
	printf("*** ERROR : illegal condition for ISOP(L != 0 && U = 0).\n");
	exit(1);
    }
...
/*	z = cuddZddGetNode(dd, index * 2 + 1, zdd_Isub0, zdd_Id); */
...
/*	y = cuddZddGetNode(dd, index * 2, zdd_Isub1, z); */
