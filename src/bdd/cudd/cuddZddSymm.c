...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddZddSymm.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  int  upper)
{
	...
	if (table->subtableZ[i].next != (unsigned) i) {
	    ...
	    do {
		(void) fprintf(stdout,"  %d", table->invpermZ[x]);
		TotalSymm++;
		gbot = x;
  ...
  int  upper)
{
    ...
    (void) fprintf(stdout,"#:G_SIFTING %8d: symmetric groups\n",symgroups);
#endif
