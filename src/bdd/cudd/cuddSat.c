...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddSat.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  DdNode * D)
{
    ...
    /* Check cache. */
    tmp = cuddCacheLookup(dd,(DdNode * (*)(DdManager *, DdNode *, DdNode *, DdNode *))Cudd_EquivDC,F,G,D);
    ...
    cuddCacheInsert(dd,(DdNode * (*)(DdManager *, DdNode *, DdNode *, DdNode *))Cudd_EquivDC,F,G,D,(res) ? One : Cudd_Not(one));
  ...
  int  pr /* verbosity level */)
{
	...
	if (ddEqualVal(cuddV(f),cuddV(g),tolerance)) {
	    ...
	    if (pr>0) {
		(void) fprintf(stdout,"Offending nodes:\n");
#if SIZEOF_VOID_P == 8
		(void) fprintf(stdout,"f: address = %lx\t value = %40.30f\n",
			       (unsigned long) f, cuddV(f));
		(void) fprintf(stdout,"g: address = %lx\t value = %40.30f\n",
			       (unsigned long) g, cuddV(g));
#else
		(void) fprintf(stdout,"f: address = %x\t value = %40.30f\n",
			       (unsigned) f, cuddV(f));
		(void) fprintf(stdout,"g: address = %x\t value = %40.30f\n",
			       (unsigned) g, cuddV(g));
#endif
	    }
	    return(0);
  ...
  int  cost)
{
	...
	(void) fprintf(stdout,"We shouldn't be here!!\n");
	return(NULL);
    }
