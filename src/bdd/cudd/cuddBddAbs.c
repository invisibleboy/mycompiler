...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddBddAbs.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  DdNode * cube)
{
    ...
    if (bddCheckPositiveCube(manager, cube) == 0) {
        (void) fprintf(stdout,"Error: Can only abstract positive cubes\n");
        return(NULL);
    }
  ...
  DdNode * cube)
{
	...
	res = cuddBddExistAbstractRecur(manager, f, cuddT(cube));
        return(res);
    }
  ...
  DdNode * cube)
{
    ...
    /* Check cache. */
    r = cuddCacheLookup(manager, Cudd_bddXorExistAbstract, f, g, cube);
    if (r != NULL) {
	return(r);
    }
    ...
    */
	...
	cuddCacheInsert(manager, Cudd_bddXorExistAbstract, f, g, cube, one);
	return(one);
    }
    cuddRef(t);
    ...
    cuddCacheInsert(manager, Cudd_bddXorExistAbstract, f, g, cube, r);
    return (r);
