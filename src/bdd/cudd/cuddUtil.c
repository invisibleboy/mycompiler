...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddUtil.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
...
static int dp2 ARGS((DdNode *f, st_table *t));
...
static double ddCountMintermAux ARGS((DdNode *node, double max, st_table *table));
  ...
  int  pr)
{
    ...
    if (f == NULL) {
	(void) fprintf(stdout,": is the NULL DD\n");
	(void) fflush(stdout);
	return(0);
    }
    azero = DD_ZERO(dd);
    ...
    if ((f == azero || f == bzero) && pr > 0){
       (void) fprintf(stdout,": is the zero DD\n");
       (void) fflush(stdout);
       return(1);
    }
    if (pr > 0) {
	...
	(void) fprintf(stdout,": %d nodes %d leaves %g minterms\n", nodes, leaves,
	    minterms);
        if (pr > 2) {
  ...
  int  nvars)
{
    ...
    table = st_init_table(st_ptrcmp,st_ptrhash);
    if (table == NULL) {
	(void) fprintf(stdout,"out-of-memory, couldn't count DD minterms.\n");
	return((double)CUDD_OUT_OF_MEM);
    }
    i = ddCountMintermAux(node,max,table);
    st_foreach(table, cuddStCountfree, NULL);
    st_free_table(table);
    if (i == (double)CUDD_OUT_OF_MEM) {
	manager->errorCode = CUDD_MEMORY_OUT;
	(void) fprintf(stdout,"out-of-memory, couldn't count DD minterms.\n");
    }
    return(i);
  ...
  DdNode * node)
{
    ...
    table = st_init_table(st_ptrcmp,st_ptrhash);
    if (table == NULL) {
	(void) fprintf(stdout,"out-of-memory, couldn't count DD paths.\n");
	return((double)CUDD_OUT_OF_MEM);
    }
  ...
  st_table * t)
{
    ...
    if (cuddIsConstant(g)) {
#if SIZEOF_VOID_P == 8
        (void) fprintf(stdout,"ID = %c0x%lx\tvalue = %-9g\n", bang(f),
		(unsigned long) g / (unsigned long) sizeof(DdNode),cuddV(g));
#else
        (void) fprintf(stdout,"ID = %c0x%x\tvalue = %-9g\n", bang(f),
...
#ifdef DD_STATS
#if SIZEOF_VOID_P == 8
    (void) fprintf(stdout,"ID = %c0x%lx\tindex = %d\tr = %d\t", bang(f),
    		(unsigned long) g / (unsigned long) sizeof(DdNode), g->index, g->ref);
#else
    (void) fprintf(stdout,"ID = %c0x%x\tindex = %d\tr = %d\t", bang(f),
    		(unsigned) g / (unsigned) sizeof(DdNode),g->index,g->ref);
#endif
#else
#if SIZEOF_VOID_P == 8
    (void) fprintf(stdout,"ID = %c0x%lx\tindex = %d\t", bang(f),
    		(unsigned long) g / (unsigned long) sizeof(DdNode),g->index);
#else
    (void) fprintf(stdout,"ID = %c0x%x\tindex = %d\t", bang(f),
    		(unsigned) g / (unsigned) sizeof(DdNode),g->index);
#endif
#endif
    n = cuddT(g);
    if (cuddIsConstant(n)) {
        (void) fprintf(stdout,"T = %-9g\t",cuddV(n));
	T = 1;
    } else {
#if SIZEOF_VOID_P == 8
        (void) fprintf(stdout,"T = 0x%lx\t",(unsigned long) n / (unsigned long) sizeof(DdNode));
#else
        (void) fprintf(stdout,"T = 0x%x\t",(unsigned) n / (unsigned) sizeof(DdNode));
#endif
	T = 0;
    }
  ...
  int * list /* current recursion path */)
{
	...
	*/
	if (node != background && node != zero) {
	    for (i = 0; i < dd->size; i++) {
		v = list[i];
		if (v == 0) (void) fprintf(stdout,"0");
		else if (v == 1) (void) fprintf(stdout,"1");
  ...
  st_table * table)
{
    ...
    Nv = cuddT(N); Nnv = cuddE(N);
    if (Cudd_IsComplement(node)) {
	Nv = Cudd_Not(Nv); Nnv = Cudd_Not(Nnv);
    }

    min1 = ddCountMintermAux(Nv, max,table) / 2.0;
    if (min1 == (double)CUDD_OUT_OF_MEM) return((double)CUDD_OUT_OF_MEM);
    min2 = ddCountMintermAux(Nnv,max,table) / 2.0;
    if (min2 == (double)CUDD_OUT_OF_MEM) return((double)CUDD_OUT_OF_MEM);
    min = min1 + min2;
