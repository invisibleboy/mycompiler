...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddExact.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  int  upper)
{
    ...
    */
    ...
    for (k = 1; k <= size; k++) {
#if DD_STATS
	(void) fprintf(stdout,"Processing subsets of size %d\n", k);
	fflush(stdout);
#endif
	newSubsets = 0;
    ...
    (void) fprintf(stdout,"\n");
#endif
    (void) fprintf(stdout,"#:S_EXACT   %8d: total subsets\n",ddTotalSubsets);
    (void) fprintf(stdout,"#:H_EXACT   %8d: total shuffles", ddTotalShuffles);
#endif
  ...
  int  upper)
{
...
#if 0
    (void) fprintf(stdout,"%d:", ddTotalShuffles);
    for (level = 0; level < numvars; level++) {
	(void) fprintf(stdout," %d", table->invperm[level]);
    }
    (void) fprintf(stdout,"\n");
#endif
    ...
    if ((ddTotalShuffles & 63) == 0) (void) fprintf(stdout,"\n");
    fflush(stdout);
#endif
#endif
...
#ifdef DD_STATS
	(void) fprintf(stdout,"New upper bound = %d\n", newBound);
	fflush(stdout);
#endif
