...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddAnneal.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  int  upper)
{
...
#ifdef DD_STATS
	(void) fprintf(stdout,"temp=%f\tsize=%d\tgen=%d\t",temp,size,maxGen);
	...
	for (innerloop = 0; innerloop < maxGen; innerloop++) {
		...
		ecount++;
#if 0
		(void) fprintf(stdout,"Exchange of %d and %d: size = %d\n",
		...
		ucount++;
#if 0
		(void) fprintf(stdout,"Jump up of %d to %d: size = %d\n",
		...
		dcount++;
#if 0
		(void) fprintf(stdout,"Jump down of %d to %d: size = %d\n",
			x,y,table->keys - table->isolated);
#endif
	    }
...
#ifdef DD_STATS
	(void) fprintf(stdout,"uphill = %d\taccepted = %d\n",
		       tosses,acceptances);
	fflush(stdout);
#endif
    }
    ...
    if (!result) return(0);
#ifdef DD_STATS
    fprintf(stdout,"#:N_EXCHANGE %8d : total exchanges\n",ecount);
    fprintf(stdout,"#:N_JUMPUP   %8d : total jumps up\n",ucount);
    fprintf(stdout,"#:N_JUMPDOWN %8d : total jumps down",dcount);
#endif
    return(1);
  ...
  double  temp)
{
	...
	(void) fprintf(stdout,"Unexpected condition in ddJumping\n");
