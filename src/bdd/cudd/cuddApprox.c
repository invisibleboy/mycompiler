...
** that says whether the node should be replaced by a constant 0. */
typedef struct NodeData {
    ...
    short int replace;		/* TRUE: replace; FLASE: don't replace */
...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddApprox.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
...
/**Function********************************************************************
  ...
  Synopsis [Extracts a dense subset from a BDD with remapping
  underapproximation method.]
  ...
  double  quality /* minimum improvement for accepted changes */)
{
	...
	(void) fprintf(stdout, "Wrong prediction: %d versus actual %d\n",
	...
	if (!Cudd_bddLeq(dd, subset, f)) {
	    (void) fprintf(stderr, "Wrong subset\n");
	}
	cuddDeref(subset);
    }
#endif
  ...
  ApproxInfo * info /* node info */)
{
	...
	(void) fprintf(stdout, "Something is wrong, ought to be in info table\n");
	return(NULL);
    }
