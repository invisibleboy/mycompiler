...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddGenetic.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  int  upper /* highest level to be reorderded */)
{
    ...
    */
    ...
    (void) fprintf(stdout,"Initial population after sifting\n");
    for (m = 0; m < popsize; m++) {
	for (i = 0; i < numvars; i++) {
	    (void) fprintf(stdout," %2d",STOREDD(m,i));
	}
	(void) fprintf(stdout," : %3d (%d)\n",STOREDD(m,numvars),repeat[m]);
    }
#endif
#endif
...
#if 0
#ifdef DD_STATS
    (void) fprintf(stdout,"Initial population before sifting\n");
    for (i = 0; i < 2; i++) {
	for (j = 0; j < numvars; j++) {
	    (void) fprintf(stdout," %2d",STOREDD(i,j));
	}
	(void) fprintf(stdout,"\n");
    }
#endif
  ...
  int  maxvar)
{
    ...
    */
    ...
    (void) fprintf(stdout,
		   ...
		   mom,dad,cut1,cut2);
    for (i = 0; i < numvars; i++) {
	if (i == cut1 || i == cut2) (void) fprintf(stdout,"|");
	(void) fprintf(stdout,"%2d ",STOREDD(mom,i));
    }
    (void) fprintf(stdout,"\n");
    for (i = 0; i < numvars; i++) {
	if (i == cut1 || i == cut2) (void) fprintf(stdout,"|");
	(void) fprintf(stdout,"%2d ",STOREDD(dad,i));
    }
    (void) fprintf(stdout,"\n");
#endif
    ...
    for (i = 0; i < numvars; i++) {
	if (i == cut1 || i == cut2) (void) fprintf(stdout,"|");
	(void) fprintf(stdout,"%2d ",STOREDD(popsize,i));
    }
    (void) fprintf(stdout,"\n");
    for (i = 0; i < numvars; i++) {
	if (i == cut1 || i == cut2) (void) fprintf(stdout,"|");
	(void) fprintf(stdout,"%2d ",STOREDD(popsize+1,i));
    }
    (void) fprintf(stdout,"\n");
#endif
