...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddClip.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  int  direction)
{
	...
	(direction ? Cudd_bddClippingAndAbstract : cuddBddClippingAndAbstract);
    if (F->ref != 1 || G->ref != 1) {
	r = cuddCacheLookup(manager, cacheOp, f, g, cube);
	if (r != NULL) {
	    return(r);
	}
    }
    ...
    */
	...
	if (F->ref != 1 || G->ref != 1)
	    cuddCacheInsert(manager, cacheOp, f, g, cube, one);
	return(one);
    }
    cuddRef(t);
    ...
    if (F->ref != 1 || G->ref != 1)
	cuddCacheInsert(manager, cacheOp, f, g, cube, r);
    return (r);
