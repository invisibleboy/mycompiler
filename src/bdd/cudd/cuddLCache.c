...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddLCache.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  DdLocalCache * cache)
{
    ...
    retval = fprintf(stdout,"Cache stats: slots = %d average = %g ", slots, mean);
    if (retval == EOF) return(0);
    retval = fprintf(stdout,"standard deviation = %g\n", stddev);
    if (retval == EOF) return(0);
    retval = fprintf(stdout,"Cache max accesses = %ld for slot %d\n", max, imax);
    if (retval == EOF) return(0);
    retval = fprintf(stdout,"Cache min accesses = %ld for slot %d\n", min, imin);
    if (retval == EOF) return(0);
    retval = fprintf(stdout,"Cache unused slots = %d\n", nzeroes);
    if (retval == EOF) return(0);

    if (totalcount) {
	expected /= totalcount;
	retval = fprintf(stdout,"Cache access hystogram for %d bins", nbins);
	if (retval == EOF) return(0);
	retval = fprintf(stdout," (expected bin value = %g)\n# ", expected);
	if (retval == EOF) return(0);
	for (i = nbins - 1; i>=0; i--) {
	    retval = fprintf(stdout,"%ld ", hystogram[i]);
	    if (retval == EOF) return(0);
	}
...
#ifdef DD_VERBOSE
    (void) fprintf(stderr,"Resizing local cache from %d to %d entries\n",
		oldslots, slots);
    (void) fprintf(stderr,"\thits = %d\tlookups = %d\thit ratio = %5.3f\n",
		cache->hits, cache->lookUps,
		(double) cache->hits / (double) cache->lookUps);
#endif
...
#pragma pointer_size short
#endif
    buckets = hash->bucket = ALLOC(DdHashItem *, numBuckets);
    MMoutOfMemory = saveHandler;
    if (buckets == NULL) {
	hash->maxsize <<= 1;
	return(1);
