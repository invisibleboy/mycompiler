...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddCompose.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  DdNode * proj)
{
    ...
    */
    ...
    /* Check cache. */
    r = cuddCacheLookup(dd,cuddBddComposeRecur,F,g,proj);
    if (r != NULL) {
	...
	*/
    ...
    cuddCacheInsert(dd,cuddBddComposeRecur,F,g,proj,r);
  ...
  DdNode * proj)
{
    ...
    /* Check cache. */
    r = cuddCacheLookup(dd,cuddAddComposeRecur,f,g,proj);
    if (r != NULL) {
	return(r);
    }
	...
	*/
    ...
    cuddCacheInsert(dd,cuddAddComposeRecur,f,g,proj,r);
...
/**Function********************************************************************
  ...
  Synopsis    [Implements the recursive step of addPermute.]
  ...
  int * permut /* permutation array */)
{
    ...
    if ((res = cuddHashTableLookup1(table,node)) != NULL) {
#ifdef DD_DEBUG
	addPermuteRecurHits++;
#endif
	return(res);
...
/**Function********************************************************************
  ...
  Synopsis    [Implements the recursive step of bddPermute.]
  ...
  int * permut /* permutation array */)
{
    ...
    /* If problem already solved, look up answer and return. */
    if ((res = cuddHashTableLookup1(table,N)) != NULL) {
#ifdef DD_DEBUG
	bddPermuteRecurHits++;
#endif
  ...
  int  lastsub)
{
    ...
    DdNode **vect1;
    DdNode **vect0;
    DdNode *(*cacheOp)(DdManager *, DdNode *, DdNode *, DdNode *);

    /* If we are past the deepest substitution, return f. */
    if (cube == DD_ONE(dd) || Cudd_IsConstant(f)) {
	return(f);
    }

    /* If problem already solved, look up answer and return. */
    cacheOp = (DdNode *(*)(DdManager *, DdNode *, DdNode *, DdNode *))
	Cudd_addNonSimCompose;
    r = cuddCacheLookup(dd,cacheOp,f,key,cube);
    if (r != NULL) {
	return(r);
    }
    ...
    */
    ...
    cuddCacheInsert(dd,cacheOp,f,key,cube,r);
