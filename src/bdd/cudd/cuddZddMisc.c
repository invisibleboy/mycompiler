...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddZddMisc.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
...
#if SIZEOF_VOID_P == 8
		    (void) fprintf(stdout, "T = 0x%lx\t",
			(unsigned long) z1_next / (unsigned long) sizeof(DdNode));
#else
		    (void) fprintf(stdout, "T = 0x%x\t",
  ...
  st_table * tab)
{
    ...
    return(1 + cuddZddDagInt(Cudd_T(n), tab) +
	cuddZddDagInt(Cudd_E(n), tab));
