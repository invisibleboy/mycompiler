...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddGroup.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  Cudd_ReorderingType method /* reordering method for the groups of leaves */)
{
    ...
    */
...
#ifdef DD_DEBUG
    if (pr > 0 && !tempTree) (void) fprintf(stdout,"cuddTreeSifting:");
    ...
    (void) fprintf(stdout,"\n");
    if (!tempTree)
	(void) fprintf(stdout,"#:IM_NODES  %8d: group tree nodes\n",
    ...
    */
	...
	(void) fprintf(stdout,"\nextsymmcalls = %d\n",extsymmcalls);
	(void) fprintf(stdout,"extsymm = %d",extsymm);
    }
    if (!tempTree && method == CUDD_REORDER_GROUP_SIFT &&
	table->groupcheck == CUDD_GROUP_CHECK7) {
	(void) fprintf(stdout,"\nsecdiffcalls = %d\n",secdiffcalls);
	(void) fprintf(stdout,"secdiff = %d\n",secdiff);
	(void) fprintf(stdout,"secdiffmisfire = %d",secdiffmisfire);
    }
#endif
  ...
  Cudd_ReorderingType method)
{
	...
	case CUDD_REORDER_GROUP_SIFT:
		...
		(void) fprintf(stdout,"Unknown group ckecking method\n");
		result = 0;
	    }
	    break;
...
#ifdef DD_DEBUG
    if (pr > 0) (void) fprintf(stdout,"ddReorderChildren:");
#endif
...
#ifdef DD_DEBUG
	if (pr > 0) (void) fprintf(stdout,"ddGroupSifting:");
#endif
    } /* for */
...
#ifdef DD_DEBUG
    if (pr > 0) (void) fprintf(stdout,"ddGroupSiftingAux from %d to %d\n",xLow,xHigh);
  ...
  Move ** moves)
{
    ...
    */
	...
	if (pr > 0 && L != checkL) {
	    (void) fprintf(stdout,"Inaccurate lower bound: L = %d checkL = %d\n",
	    L, checkL);
	}
#endif
...
#ifdef DD_DEBUG
	    if (pr > 0) (void) fprintf(stdout,"ddGroupSiftingUp (2 single groups):\n");
#endif
...
#ifdef DD_DEBUG
            if (pr > 0) (void) fprintf(stdout,"ddGroupSiftingDown (2 single groups):\n");
#endif
            if ((double) size > (double) limitSize * table->maxGrowth)
                return(1);
            if (size < limitSize) limitSize = size;
  ...
  Move ** moves)
{
	...
	(void) fprintf(stdout,"Missed local minimum: initialSize:%d  bestSize:%d  finalSize:%d\n",initialSize,bestSize,size);
#endif
    ...
    table->subtables[x].next = newxtop; /* x is bottom of its group, join */
                                    /* it to top of its group */
#ifdef DD_DEBUG
    if (pr > 0) (void) fprintf(stdout,"ddGroupMove:\n");
#endif
  ...
  int  y)
{
    ...
    table->subtables[x].next = newxtop; /* x is bottom of its group, join */
                                    /* to its top */
#ifdef DD_DEBUG
    if (pr > 0) (void) fprintf(stdout,"ddGroupMoveBackward:\n");
#endif
  ...
  int  size)
{
	...
	(table->subtables[move->y].next == move->y)) {
            ...
            if (!res) return(0);
#ifdef DD_DEBUG
            if (pr > 0) (void) fprintf(stdout,"ddGroupSiftingBackward:\n");
  ...
  int  y)
{
	...
	if (cuddTestInteract(table,xindex,yindex)) {
#if defined(DD_DEBUG) && defined(DD_VERBOSE)
	    (void) fprintf(stdout,"Second difference for %d = %g Pos(%d)\n",
...
#if defined(DD_DEBUG) && defined(DD_VERBOSE)
    if (res) {
	(void) fprintf(stdout,"Found extended symmetry! x = %d\ty = %d\tPos(%d,%d)\n",
	    xindex,yindex,x,y);
    }
#endif
