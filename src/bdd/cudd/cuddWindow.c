...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddWindow.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
  ...
  Cudd_ReorderingType submethod /* window reordering option */)
{
	...
	if (table->keys - table->isolated != (unsigned) supposedOpt) {
	    (void) fprintf(stdout,"Convergence failed! (%d != %d)\n", table->keys
	    - table->isolated, supposedOpt);
	}
#endif
	break;
...
#ifdef DD_STATS
	if (newevent) {(void) fprintf(stdout,"|"); fflush(stdout);}
#endif
    } while (newevent);
