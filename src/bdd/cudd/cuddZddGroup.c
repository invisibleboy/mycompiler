...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddZddGroup.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
...
#ifdef DD_DEBUG
    if (pr > 0 && !tempTree) (void) fprintf(stdout,"cuddZddTreeSifting:");
...
#ifdef DD_DEBUG
    if (pr > 0) (void) fprintf(stdout,"zddReorderChildren:");
#endif
...
#ifdef DD_DEBUG
	if (pr > 0) (void) fprintf(stdout,"zddGroupSifting:");
#endif
    } /* for */
...
#ifdef DD_DEBUG
    if (pr > 0) (void) fprintf(stdout,"zddGroupSiftingAux from %d to %d\n",xLow,xHigh);
...
#ifdef DD_DEBUG
	    if (pr > 0) (void) fprintf(stdout,"zddGroupSiftingUp (2 single groups):\n");
#endif
...
#ifdef DD_DEBUG
            if (pr > 0) (void) fprintf(stdout,"zddGroupSiftingDown (2 single groups):\n");
#endif
            if ((double) size > (double) limitSize * table->maxGrowth)
                return(1);
            if (size < limitSize) limitSize = size;
  ...
  Move ** moves)
{
    ...
    table->subtableZ[x].next = newxtop; /* x is bottom of its group, join */
                                    /* it to top of its group */
#ifdef DD_DEBUG
    if (pr > 0) (void) fprintf(stdout,"zddGroupMove:\n");
#endif
  ...
  int  y)
{
    ...
    table->subtableZ[x].next = newxtop; /* x is bottom of its group, join */
                                    /* to its top */
#ifdef DD_DEBUG
    if (pr > 0) (void) fprintf(stdout,"zddGroupMoveBackward:\n");
#endif
  ...
  int  size)
{
	...
	(table->subtableZ[move->y].next == move->y)) {
            ...
            if (!res) return(0);
#ifdef DD_DEBUG
            if (pr > 0) (void) fprintf(stdout,"zddGroupSiftingBackward:\n");
