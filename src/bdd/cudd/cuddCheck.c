...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddCheck.c,v 1.1.1.1 2005/04/18 17:40:17 mchu Exp $";
#endif
...
#pragma pointer_size restore
	...
	if (i != (unsigned) table->perm[index]) {
	    (void) fprintf(stdout,"Permutation corrupted: invperm[%d] = %d\t perm[%d] = %d\n", i, index, index, table->perm[index]);
	}
	...
	for (j = 0; j < slots; j++) {	/* for each subtable slot */
		...
		if (cuddT(f) != NULL && cuddE(f) != NULL && f->ref != 0) { 
			...
			(void) fprintf(stdout,"Error: node has illegal index\n");
			cuddPrintNode(f);
			flag = 1;
		    }
			...
			<= i) {
			(void) fprintf(stdout,"Error: node has illegal children\n");
			cuddPrintNode(f);
			flag = 1;
		    }
		    if (Cudd_Regular(cuddT(f)) != cuddT(f)) {
			(void) fprintf(stdout,"Error: node has illegal form\n");
			cuddPrintNode(f);
			flag = 1;
		    }
		    if (cuddT(f) == cuddE(f)) {
			(void) fprintf(stdout,"Error: node has identical children\n");
			cuddPrintNode(f);
			flag = 1;
		    }
		    if (cuddT(f)->ref == 0 || Cudd_Regular(cuddE(f))->ref == 0) {
			(void) fprintf(stdout,"Error: live node has dead children\n");
			cuddPrintNode(f);
			flag =1;
		    }
		    ...
		    */
		    ...
		    fprintf(stdout,"Error: node has illegal Then or Else pointers\n");
		    cuddPrintNode(f);
		    flag = 1;
		}
	...
	if ((unsigned) totalNode != table->subtables[i].keys) {
	    fprintf(stdout,"Error: wrong number of total nodes\n");
	    flag = 1;
	}
	if ((unsigned) deadNode != table->subtables[i].dead) {
	    fprintf(stdout,"Error: wrong number of dead nodes\n");
	    flag = 1;
	}
	...
	if (i != (unsigned) table->permZ[index]) {
	    (void) fprintf(stdout,"Permutation corrupted: invpermZ[%d] = %d\t permZ[%d] = %d in ZDD\n", i, index, index, table->permZ[index]);
	}
	...
	for (j = 0; j < slots; j++) {	/* for each subtable slot */
		...
		if (cuddT(f) != NULL && cuddE(f) != NULL && f->ref != 0) { 
		    if ((int) f->index != index) {
			(void) fprintf(stdout,"Error: ZDD node has illegal index\n");
			cuddPrintNode(f);
			flag = 1;
		    }
		    if (Cudd_IsComplement(cuddT(f)) ||
			Cudd_IsComplement(cuddE(f))) {
			(void) fprintf(stdout,"Error: ZDD node has complemented children\n");
			cuddPrintNode(f);
			flag = 1;
		    }
		    if ((unsigned) cuddIZ(table,cuddT(f)->index) <= i ||
		    (unsigned) cuddIZ(table,cuddE(f)->index) <= i) {
			(void) fprintf(stdout,"Error: ZDD node has illegal children\n");
			cuddPrintNode(f);
			cuddPrintNode(cuddT(f));
			cuddPrintNode(cuddE(f));
			flag = 1;
		    }
		    if (cuddT(f) == DD_ZERO(table)) {
			(void) fprintf(stdout,"Error: ZDD node has zero then child\n");
			cuddPrintNode(f);
			flag = 1;
		    }
		    if (cuddT(f)->ref == 0 || cuddE(f)->ref == 0) {
			(void) fprintf(stdout,"Error: ZDD live node has dead children\n");
			cuddPrintNode(f);
			flag =1;
		    }
		    /* Increment the internal reference count for the
		    ...
		    */
		    ...
		    fprintf(stdout,"Error: ZDD node has illegal Then or Else pointers\n");
		    cuddPrintNode(f);
		    flag = 1;
		}
	...
	if ((unsigned) totalNode != table->subtableZ[i].keys) {
	    fprintf(stdout,"Error: wrong number of total nodes in ZDD\n");
	    flag = 1;
	}
	if ((unsigned) deadNode != table->subtableZ[i].dead) {
	    fprintf(stdout,"Error: wrong number of dead nodes in ZDD\n");
	    flag = 1;
	}
	...
	while (f != NULL) {
		...
		if (f->index != CUDD_MAXINDEX) {
		    fprintf(stdout,"Error: node has illegal index\n");
#if SIZEOF_VOID_P == 8
		    fprintf(stdout,"       node 0x%lx, id = %d, ref = %d, value = %g\n",(unsigned long)f,f->index,f->ref,cuddV(f));
#else
		    fprintf(stdout,"       node 0x%x, id = %d, ref = %d, value = %g\n",(unsigned)f,f->index,f->ref,cuddV(f));
#endif
		    flag = 1;
		}
	    } else {
	...
	(void) fprintf(stdout,"Error: wrong number of total nodes in constants\n");
	flag = 1;
    }
    if ((unsigned) deadNode != table->constants.dead) {
	(void) fprintf(stdout,"Error: wrong number of dead nodes in constants\n");
	flag = 1;
    }
...
#if SIZEOF_VOID_P == 8
	    fprintf(stdout,"ref count error at node 0x%lx, count = %d, id = %d, ref = %d, then = 0x%lx, else = 0x%lx\n",(unsigned long)f,count,f->index,f->ref,(unsigned long)cuddT(f),(unsigned long)cuddE(f));
#else
	    fprintf(stdout,"ref count error at node 0x%x, count = %d, id = %d, ref = %d, then = 0x%x, else = 0x%x\n",(unsigned)f,count,f->index,f->ref,(unsigned)cuddT(f),(unsigned)cuddE(f));
#endif
...
#pragma pointer_size restore
	...
	if (keys != 0) {
	    (void) fprintf(stderr, "Wrong number of keys found \
in unique table %d (difference=%d)\n", i, keys);
	    count++;
	}
	if (dead != 0) {
	    (void) fprintf(stderr, "Wrong number of dead found \
...
in the constant table (difference=%d)\n", dead);
	...
	(void) fprintf(stderr, "Wrong number of total keys found \
(difference=%d)\n", totalKeys-table->keys);
    }
    if ((unsigned) totalSlots != table->slots) {
	(void) fprintf(stderr, "Wrong number of total slots found \
(difference=%d)\n", totalSlots-table->slots);
    }
    if (table->minDead != (table->gcPercent * table->slots) / 100) {
	(void) fprintf(stderr, "Wrong number of minimum dead found \
(%d vs. %d)\n", table->minDead, (table->gcPercent * table->slots) / 100);
    }
    if ((unsigned) totalDead != table->dead + table->deadZ) {
	(void) fprintf(stderr, "Wrong number of total dead found \
...
/**Function********************************************************************
  ...
  Description [Prints to stdout the number of live nodes for each
  ...
  DdManager * dd)
{
    ...
    /* Print header. */
#if SIZEOF_VOID_P == 8
    retval = fprintf(stdout,"*** DD heap profile for 0x%lx ***\n", (unsigned long) dd);
#else
    retval = fprintf(stdout,"*** DD heap profile for 0x%x ***\n", (unsigned) dd);
#endif
	...
	if (nodes) {
	    nonempty++;
	    retval = fprintf(stdout,"%5d: %5d nodes\n", i, nodes);
	    if (retval == EOF) return 0;
	    if (nodes > maxnodes) {
		maxnodes = nodes;
		largest = i;
    ...
    if (nodes) {
	nonempty++;
	retval = fprintf(stdout,"const: %5d nodes\n", nodes);
	if (retval == EOF) return 0;
	if (nodes > maxnodes) {
	    maxnodes = nodes;
    ...
    /* Print summary. */
    retval = fprintf(stdout,"Summary: %d tables, %d non-empty, largest: %d ",
	  ntables+1, nonempty, largest);
    if (retval == EOF) return 0;
    retval = fprintf(stdout,"(with %d nodes)\n", maxnodes);
    if (retval == EOF) return 0;
...
#if SIZEOF_VOID_P == 8
		    (void) fprintf(stdout,"parent is at 0x%lx, id = %d, ref = %d, then = 0x%lx, else = 0x%lx\n",
			(unsigned long)f,f->index,f->ref,(unsigned long)cuddT(f),(unsigned long)cuddE(f));
#else
		    (void) fprintf(stdout,"parent is at 0x%x, id = %d, ref = %d, then = 0x%x, else = 0x%x\n",
			(unsigned)f,f->index,f->ref,(unsigned)cuddT(f),(unsigned)cuddE(f));
#endif
		}
		f = f->next;
  ...
  DdNode * node)
{
	...
	for (j=0;j<slots;j++) {
		...
		if ((Cudd_Regular(cuddE(f)) == node || cuddT(f) == node) && f->ref != 0) {
		    (void) fprintf(stdout,"error with zero ref count\n");
		    (void) fprintf(stdout,"parent is 0x%x, id = %d, ref = %d, then = 0x%x, else = 0x%x\n",f,f->index,f->ref,cuddT(f),cuddE(f));
		    (void) fprintf(stdout,"child  is 0x%x, id = %d, ref = %d, then = 0x%x, else = 0x%x\n",node,node->index,node->ref,cuddT(node),cuddE(node));
		}
		f = f->next;
	    }
	}
